...
class BITStar:
    ...
    def planning(self):
        ...
        for k in range(500):
            ...
            if  self.g_T[self.x_goal] >40:
                self.cost_list.append(40)
            else:
                self.cost_list.append(self.g_T[self.x_goal])                
            if not self.Tree.QE and not self.Tree.QV:
                # just to choose the value of m
                if k == 0:
                    m = 350
                else:
                    m = 200

                # only when path to goal is found
                if self.x_goal.parent is not None:
                    path_x, path_y = self.ExtractPath()
                    plt.plot(path_x, path_y, linewidth=2, color='r')
                    plt.pause(0.5)

                # pruning edges and vertices for getting better solution
                self.Prune(self.g_T[self.x_goal])    # g_T - cost to come to current state from start given current tree
                self.X_sample.update(self.Sample(m, self.g_T[self.x_goal], cMin, xCenter, C))   # sampling function based on cBest
            ...
            self.no_of_nodes.append(len(self.Tree.V))

        path_x, path_y = self.ExtractPath()
        print("x-coordinates of path: ", path_x)
        print("y-coordinates of path: ", path_y)
        plt.plot(path_x, path_y, linewidth=2, color='r')
        plt.pause(0.01)
        plt.show()
        plt.close()
    ...
    def Prune(self, cBest):
        self.X_sample = {x for x in self.X_sample if self.f_estimated(x) < cBest}        # only keeping the sample points which cost is less than cBest
        self.Tree.V = {v for v in self.Tree.V if self.f_estimated(v) <= cBest}           # only keeping vertices which cost less than or equal to cBest
    ...
    def f_estimated(self, node):                    # f_estimated - cost of path from start to goal
